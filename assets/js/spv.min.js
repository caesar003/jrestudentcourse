$(document).ready(function(){
  ...
  $('#mystudents').dataTable({
      "ajax" :{
        "url":`${siteurl}/student/student_data`,
        "dataSrc":""
      },
      "columns":[
        {
          ...
          "render":function(data,type,row){
            if(data.grp!=''){
             if(data.cnst2!=''&&data.cnst3!=''&&data.cnst4!=''){
                return `<a href="${siteurl}/student_single?pin=${data.pin}" class="tooltip-bottom" title="${data.complete_name} - ${data.cnst2} - ${data.cnst3} - ${data.cnst4}"> ${data.grp} </a>`;

              } else if(data.cnst2!=''&&data.cnst3!=''){
                return `<a href="${siteurl}/student_single?pin=${data.pin}" class="tootip-bottom" title="${data.complete_name} - ${data.cnst2} - ${data.cnst3}"> ${data.grp} </a>`;

              } else{
                return `<a href="${siteurl}/student_single?pin=${data.pin}" title="${data.complete_name} - ${data.cnst2}" class="tooltip-bottom"> ${data.grp} </a>`;
              }
            } else {
              if(data.cnst2!=''){
                if(data.cnst3!=''){
                  if(data.cnst4!=''){
                    return `<a href="${siteurl}/student_single?pin=${data.pin}" title="${data.complete_name} - ${data.cnst2} - ${data.cnst3}-${data.cnst4}">${data.complete_name}</a>`;
                  } else {
                    return `<a href="${siteurl}/student_single?pin=${data.pin}" title="${data.complete_name} - ${data.cnst2} - ${data.cnst3}">${data.complete_name}</a>`;
                  }
                } else{
                  return `<a href="${siteurl}/student_single?pin=${data.pin}" title="${data.complete_name} - ${data.cnst2}">${data.complete_name}</a>`;
                }
              } else {
                return `<a href="${siteurl}/student_single?pin=${data.pin}">${data.complete_name}</a>`;
              }
            }
          }
        },
         ...
         "data" : {pin:"pin", complete_name:"complete_name", cnst2:"cnst4", cnst3:"cnst3", cnst4:"cnst4", nick_name:"nick_name", nnst2:"nnst2", nnst3:"nnst3", nnst4:"nnst4"},
          "render" :function(data,type,row){
            if(data.cnst2!=''){
              if(data.cnst3!=''){
                if(data.cnst4!=''){
                  return `<a href="${siteurl}/student_single?pin=${data.pin}"> ${data.nick_name} -${data.nick_name} - ${data.nnst2} - ${data.cnst3} - ${data.cnst4} </a>`;
                } else {
                  return `<a href="${siteurl}/student_single?pin=${data.pin}"> ${data.nick_name} - ${data.nnst2}- ${data.nnst3} </a>`;
                }
              }else {
                return `<a href="${siteurl}/student_single?pin=${data.pin}"> ${data.nick_name} - ${data.nnst2} </a>`;
              }
            } else {
              return `<a href="${siteurl}/student_single?pin=${data.pin}">${data.nick_name}</a>`;
            }
          }
        },
        {
          ...
          "render":function(data,type,row){
            if(data.adrst2==''){
              return `<a href="${siteurl}/student_single?pin=${data.pin}">${data.address}</a>`;
            } else{
              if(data.adrst3==''){
                return `<a href="${siteurl}/student_single?pin=${data.pin}" title="${data.address} || ${data.adrst2}">${data.address.substr(0,15)}.....</a>`;
              } else {
                if (data.adrst4==''){
                  return `<a href="${siteurl}/student_single?pin=${data.pin}" title="${data.address} || ${data.adrst2} || ${data.adrst3}">${data.address.substr(0,15)}.....</a>`;
                } else {
                  return `<a href="${siteurl}/student_single?pin=${data.pin}" title="${data.address} || ${data.adrst2} || ${ data.adrst3} || ${data.adrst4}">${data.address.substr(0,15)}.....</a>`;
                }
              }
            }
          }
        },
        {
          ...
          "render" : function (data, type, row){
            if(data.cnst2==''){
              return `<a href="${siteurl}/student_single?pin=${data.pin}">${$.format.date(data.date_of_birth, "MMM/dd/yyyy")}`;
            } else {
              if(data.cnst3==''){
                return `<a href="${siteurl}/student_single?pin=${data.pin}">${$.format.date(data.date_of_birth, "MMM/dd/yyyy")} - ${$.format.date(data.dobst2, "MMM/dd/yyyy")}`;
              } else {
                if(data.cnst4==''){
                  return `<a href="${siteurl}/student_single?pin=${data.pin}">${$.format.date(data.date_of_birth, "MMM/dd/yyyy")} - ${$.format.date(data.dobst2, "MMM/dd/yyyy")} - ${$.format.date(data.dobst3, "MMM/dd/yyyy")}`;
                } else {
                  return `<a href="${siteurl}/student_single?pin=${data.pin}">${$.format.date(data.date_of_birth, "MMM/dd/yyyy")} - ${$.format.date(data.dobst2, "MMM/dd/yyyy")} - ${$.format.date(data.dobst3, "MMM/dd/yyyy")} - ${$.format.date(data.dobst4, "MMM/dd/yyyy")}` ;
                }
              }
            }
          }
        },
        {
          ...
          "render": function(data,type,row){
            if(data.phst2==''){
              return `<a href="${siteurl}/student_single?pin=${data.pin}">${data.phone}`;
            } else{
              if(data.phst3==''){
                return `<a href="${siteurl}/student_single?pin=${data.pin}">${data.phone} - ${data.phst2}`;
              } else {
                if(data.phst4==''){
                  return `<a href="${siteurl}/student_single?pin=${data.pin}">${data.phone} - ${data.phst2} - ${data.phst3}`;
                } else {
                  return `<a href="${siteurl}/student_single?pin=${data.pin}">${data.phone} - ${data.phst2} - ${data.phst3} -${data.phst4}`;
                }
              }
            }
          }
        },
        {
          ...
          "render" : function(data, meta, row){
            return `<a href="${siteurl}/student_single?pin=${data.pin}">${data.program}</a>`
          }
        },
        {
          "data" : {pin:"pin", starting_date:"starting_date"},
          "render" : function (data, type, row){
            return `<a href="${siteurl}/student_single?pin=${data.pin}">${$.format.date(data.starting_date, "MMM/dd/yyyy")}</a>`;
          }
        },
        {
          ...
          "render" : function (data, type, row, meta){
            if(data.length>20){
              return `<a href="${siteurl}/student_single?pin=${data.pin}" title="${data.bground}">${data.bground.substr(0,15)}...</a>`;
            } else {
              return `<a href="${siteurl}/student_single?pin=${data.pin}">${data.bground}</a>`
            }
          }
        }
      ]
  ...
  $('#btn_delete_student').on('click', function(){
       ...
       $.ajax({
          type: "post",
          url: `${siteurl}/student/delete_student`,
          dataType : "json",
          data : {pin: pin},
   ...
   $('#myaft').dataTable({
    "ajax" : {
      "url" :`${siteurl}/student/after_teaching_data`,
      "dataSrc" : ""
    },
    "columns" :
    [
      {
          "data" : "pin",
          "render" : function(data, meta, row){
            return `<a href="${siteurl}/student_single?pin=${data}">${data}</a>`
          }
      },
          ...
          "render":function(data,type,row){
            if(data.grp!=''){
             if(data.cnst2!=''&&data.cnst3!=''&&data.cnst4!=''){
                ...
                return `<a title="${data.complete_name} - ${data.cnst2}" class="tooltip-bottom" href="${siteurl}/student_single?pin=${data.pin}"> ${data.grp}</a>`;
              }
            } else {
              if(data.cnst2!=''){
                if(data.cnst3!=''){
  ...
  function check_pin(grp,pn,cn,nn,ad,pb,db,ph,cn2,nn2,ad2,pb2,db2,ph2,cn3,nn3,ad3,pb3,db3,ph3,cn4,nn4,ad4,pb4,db4,ph4,pr,pd,sd,re,ta,di,bg,si,wp,ap){
    var bck = 'background-color',
        clr ='#fbe2e6';
    $.ajax({
      url: `${siteurl}/student/pin_availability`,
      type: 'post',
      data:{pin:pn},
