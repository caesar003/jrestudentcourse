  ...
  <!-- EDIT FSP ITEM -->
                    ...
                    <div class="form-row"> 
                      <div class="form-group col-5"> 
                        ...
                        <div class="input-group"> 
                          ...
                          <select class="form-control" name="pr2" id="pr2" required disabled> 
    ...
    <script type = "text/javascript" >
        ...
        $('#mycourse').DataTable({
          ...
          "columns": [
            ...
            {"data" : "s"},
            {
              ...
              "render" : function(data, type, row, meta){
                return '<a title="Edit" href="javascript:void(0);" class="btn btn-info btn-sm item_edit tooltip-right" data-meeting="'+data.meeting+'" data-course_date="'+data.course_date+'" data-teacher="'+data.teacher+'" data-duration="'+data.duration+'" data-material="'+data.material+'" data-evaluation="'+data.evaluation+'" data-w="'+data.w+'" data-s="'+data.s+'" data-test="'+data.test+'" data-test_number="'+data.test_number+'" data-test_name="'+data.test_name+'" data-of_test_number="'+data.of_test_number+'" data-of_test="'+data.of_test+'"><i class="fas fa-pencil-alt"></i></a>'
              }
            }
          ]
        });
      ...
      $('#btn_save').on('click', function(){
        var p="<?php echo $pin;?>",m=$('#me').val(),cd=$('#cd').val(),tc=$('#tc').val(),du=$('#du').val(), ma=$('#ma').val(),ev=$('#ev').val(), w = $('#wr').val(),s=$('#sp').val(),test = '', tnu = $('#tnu').val(), tn = $('#tn').val(), otn = $('#otn').val(), ot = $('#ot').val(), after_teaching = 'yes', bgc = 'background-color', clr = 'pink';
        if (m == '' || cd == '' || tc == '' || du == '' || ma == '' || ev == ''){
          if(m == ''){
            $('#me').css(bgc,clr);
          }
          ...
          if(isNaN(m)){
            ...
            $('#me').css(bgc,clr);
            $('#nsef').addClass('alert alert-danger');
            $('#nsef').html("Meeting field must only be numbers!");
          } else {
            if(isNaN(w)){
              console.log('writing not number');
              $('#wr').css(bgc,clr);
              $('#nsef').addClass('alert alert-danger');
              $('#nsef').html("Assessment must only be numbers!");
            } else {
              if (isNaN(s)){
                  ...
                  success: function (response){
                    if (response == 'true'){
                      ...
                      if ($('#test').is(':checked')){
                              ...
                              $.ajax({
                                ...
                                success : function (response){
                                  if(response == 'true'){
                                    ...
                                    insert_form(p, m, cd, tc, du, ma, ev, w, s, test, tnu, tn, otn, ot);
                        ...
                        insert_form(p, m, cd, tc, du, ma, ev, w, s, test, tnu, tn, otn, ot);
                        set_after_teaching(p, after_teaching);
                      }
                    }
                  }
                });
      ...
      function insert_form(a, b, c, d, e, f, g, h, i, j, k, l, m, n){
        $.ajax({
          type: "POST",
      ...
      function set_after_teaching(a, b){
        $.ajax({
          type: "POST",
      ...
      $('#show_data').on('click', '.item_edit', function() {
        var b=$(this).data('meeting'),c=($.format.date($(this).data('course_date'), "yyyy-MM-dd\THH:mm")),d=$(this).data('teacher'),e=$(this).data('duration'),f=$(this).data('material'),g=$(this).data('evaluation'),w=$(this).data('w'),s=$(this).data('s'),j=$(this).data('test'),k=$(this).data('test_number'),l=$(this).data('test_name'),m=$(this).data('of_test_number'),n=$(this).data('of_test'),o='';        
          ...
          if (isNaN(w) || isNaN(s)) {
            ...
            if(isNaN(du)){
              ...
              $.ajax({
                ...
                url: "<?php echo site_url('student_single/set_aft')?>",
                dataType: "JSON",
                data: {p: p, after_teaching: after_teaching},
                success: function(data) {
